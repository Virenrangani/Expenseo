...
class SplitDataSourceImpl implements SplitDataSource {
  ...
  Future<List<SplitModel>> getSplitExpenses(String groupId) async {
    try {
      final splitExpense = await firestore
          .collection('groups')
          .doc(groupId)
          .collection('expense')
      ...
      return splitExpense.docs
          .map((e) => SplitModel.fromJson(e.data(), e.id))
          .toList();
    } on FirebaseException catch (e) {
      throw Exception(AppErrors.handleFireStoreException(e));
    } catch (e) {
  ...
  Future<void> deleteGroup(String groupId) async {
    try {
      final groupExpense = await firestore
          .collection('groups')
          .doc(groupId)
          .collection('expense')
          .get();
      ...
      final groupSettlement = await firestore
          .collection('groups')
          .doc(groupId)
  ...
  Future<List<SettleBalanceModel>> getSettlements(String groupId) async {
    try {
      final settlement = await firestore
          .collection('groups')
          .doc(groupId)
          .collection('settlements')
          .orderBy('createdAt', descending: true)
      ...
      return settlement.docs
          .map((e) => SettleBalanceModel.fromJson(e.id, e.data()))
          .toList();
    } on FirebaseException catch (e) {
      throw Exception(AppErrors.handleFireStoreException(e));
    } catch (e) {
